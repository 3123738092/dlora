#!/usr/bin/env bash
# 2-GPU parallel CFR-RAFT fine-tuned UDM10 evaluation
set -euo pipefail
cd "$(dirname "$0")/.."
...
LOG=runs/cfr_raft_finetune/udm10_eval_2gpu.log

echo "==== CFR-RAFT FT UDM10 Eval (2-GPU) ====" | tee "$LOG"
...
run_gpu() {
    local gpu=$1; shift
    local log="/tmp/udm10_gpu${gpu}.log"
    export CUDA_VISIBLE_DEVICES=$gpu
    for clip in "$@"; do
        name=$(basename "$clip" .mp4)
        echo "[GPU$gpu $(date +%H:%M:%S)] Processing $name ..." | tee -a "$LOG" "$log"
        "$PY" src/test_DLoRAL.py             --input_image "$clip"             --output_dir "$OUT"             --pretrained_path "$CKPT"             --load_cfr             --flow_estimator raft --mixed_precision no 2>&1 | tail -5 | tee -a "$log"
